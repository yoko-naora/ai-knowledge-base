...
// No hardcoded access code — missing env var → welcome email skips code block.
//
// TEST:
...
export async function onRequestPost(context) {
  ...
  const accessCode = context.env.ACCESS_CODE; // From env var, no default
  ...
  if (type === "checkout.session.completed") {
    ...
    html = buildEmail({ isJp, headerTitle: isJp ? "🎉 購読完了！AI知識庫へようこそ" : "🎉 订阅完成！欢迎加入 AI知識庫", headerSub: isJp ? "ご購読ありがとうございます。以下があなたの特典とアクセス方法です。" : "感谢您的订阅。以下是您的权益和访问方法。", planName, features: featuresHtml(isMonthly, isJp), showAccessCode: true, showCta: true, accessCode });
